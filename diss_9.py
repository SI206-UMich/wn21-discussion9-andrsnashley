...
def getCharmanderLink(soup):
    ...
    anchor2 = soup.find_all('span', class_='infocard-lg-img')[3]
    anchor3 = anchor2.find('a')['href']
    return 'https://pokemondb.net' + anchor3

# Task 2: Get the details from the box below "Egg moves". Get all the move names and store them into a list. The function should return that list of moves.
def getEggMoves(pokemon):
    ...
    rows = anchor1.find_all("tr")
    eggMoves = []
    for x in rows:
        anchor2 = x.find('a', class_="ent-name")
        if anchor2 != None:
            move = anchor2.text
